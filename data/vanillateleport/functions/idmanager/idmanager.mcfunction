...
scoreboard players add @e[tag=p] pid 0
#players who went offline and have invalid IDs are reset to 0
execute as @e[tag=p,scores={offline=1..}] run scoreboard players set @s id 0
#players with ID 0 are assigned valid unique IDs
execute as @e[tag=p,scores={id=0}] run function vanillateleport:idmanager/login
execute as @e[tag=p,scores={pid=0}] run function vanillateleport:idmanager/assignpid
